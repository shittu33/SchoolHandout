...
public final class FlipPdfForwardLoaderTask implements Runnable {
    private static FlipPdfForwardLoaderTask _object;
    ...
    public static FlipPdfForwardLoaderTask get(Activity context) {
        if (_object == null) {
            _object = new FlipPdfForwardLoaderTask(context);
        }
        return _object;
    }

    private FlipPdfForwardLoaderTask(Activity context) {
        if (context instanceof FlipPdfActivity) {
            this.flipPdfActivity = (FlipPdfActivity) context;
        }
    }

    FlipPdfBackLoaderTask.OnPdfPageDownloadListener onPdfPageDownloadListener;

    public void setOnPdfPageDownloadListener(FlipPdfBackLoaderTask.OnPdfPageDownloadListener onPdfPageDownloadListener) {
...
//            }
            ...
            if (!page_file.exists()) {
                final File saved_page = PdfUtils.SavePdfPage_to(flipPdfActivity, pdf_path, page_file.getPath(), i);
                if (saved_page != null) {
                    final int page_no = i;
...
//                flipPdfActivity.runOnUiThread(new Runnable() {
//                    @Override
//                    public void run() {
//                        Runnable runnable = new Runnable() {
//                            public void run() {
//                                Log.i("pdf_bitmap", "requested index: about to request " + requested_index + "/" + pdf_page_count);
////                                flipPdfActivity.mAdapterFlipView.setSelection(requested_index);
//                                flipPdfActivity.ManuallyRefresh_Page(50);
