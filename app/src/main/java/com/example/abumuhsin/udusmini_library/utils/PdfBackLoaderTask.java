...
public final class PdfBackLoaderTask implements Runnable {
    private static PdfBackLoaderTask _object;
    private Thread pdf_thread;
    private FlipBooKActivity flipBooKActivity;
    ...
    public static PdfBackLoaderTask get(Activity context) {
        if (_object == null) {
            _object = new PdfBackLoaderTask(context);
        }
        return _object;
    }

    private PdfBackLoaderTask(Activity context) {
        if (context instanceof FlipBooKActivity) {
            this.flipBooKActivity = (FlipBooKActivity) context;
        }
//        pdf_path = flipBooKActivity.pdf_path;
    }
...
//        this.page_path = page_path;
    ...
    private void LoadBackward(/*PdfDocument pdfDocument, PdfiumCore pdfiumCore*/) {
        pdf_page_count = PdfUtils.getPdfPage_count(flipBooKActivity, pdf_path);
        ...
        if (requested_index >= 0) {
            ...
            for (int i = start_index; i >= 0; i--) {
                ...
                File page_file = null;
//                if (flipBooKActivity.other_app_intent!=null) {
//                    page_file = create_new_File_for_ActivePdf(pdf_name+"_dir", i);
//                }else {
                    page_file = create_new_File_for_ActivePdf(pdf_name, i);
//                }
                if (!page_file.exists()) {
                    final File page_saved =PdfUtils.SavePdfPage_to(flipBooKActivity, pdf_path, page_file.getPath(), i);
                    if (page_saved != null) {
                        final int page_no = i;
                        ...
                        flipBooKActivity.runOnUiThread(runnable);
                    }
//                    saveImage(getBitmapFromPdf(pdfDocument, pdfiumCore, i), page_file);
    ...
    Bitmap getBitmapFromPdf(PdfDocument pdfDocument, PdfiumCore pdfiumCore, int i) {
        ...
        Bitmap bitmap = Bitmap.createBitmap(getPdfScaledwidth(flipBooKActivity, width), getPdfScaledheight(flipBooKActivity, height),
                Bitmap.Config.ARGB_8888);
        pdfiumCore.renderPageBitmap(pdfDocument, bitmap, i, 0, 0,
                getPdfScaledwidth(flipBooKActivity, width), getPdfScaledheight(flipBooKActivity, height), true);
        return bitmap;
    }
