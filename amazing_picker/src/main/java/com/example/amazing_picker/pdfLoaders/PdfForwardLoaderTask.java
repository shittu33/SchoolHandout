...
public final class PdfForwardLoaderTask implements Runnable {
    private static PdfForwardLoaderTask _object;
    ...
    public static PdfForwardLoaderTask get(Activity context) {
        if (_object == null) {
            _object = new PdfForwardLoaderTask(context);
        }
        return _object;
    }

    private PdfForwardLoaderTask(Activity context) {
    ...
    public void LoadPdfToStorage(String pdf_path,int start_index) {
    ...
    public void run() {
        ...
        final int prev_index = start_index >= 0 ? start_index: 0;// ex 26-3 = 23
        int start_no = pdf_page_count-1 >= prev_index ? prev_index : pdf_page_count-1;
        for (int i = start_no; i < pdf_page_count; i++) {
                if (do_stop_thread) {
