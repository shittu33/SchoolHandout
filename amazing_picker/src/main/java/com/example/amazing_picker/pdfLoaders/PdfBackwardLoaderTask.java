...
public final class PdfBackwardLoaderTask implements Runnable {
    private static PdfBackwardLoaderTask _object;
    ...
    public static PdfBackwardLoaderTask get(Activity context) {
        if (_object == null) {
            _object = new PdfBackwardLoaderTask(context);
        }
        return _object;
    }

    private PdfBackwardLoaderTask(Activity context) {
    ...
    public void run() {
        ...
        if (start_index >= 0) {
            int start_no =  start_index- 1 > 0 ? start_index - 1 : start_index;
            for (int i = start_no; i >=0; i--) {
                    if (do_stop_thread) {
