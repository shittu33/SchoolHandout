...
public class PageFlipView extends GLSurfaceView implements Renderer {
    public AdapterPageFlipView adapterPageFlipView;
    ...
    public PageFlipView(Context context, AdapterPageFlipView adapterPageFlipView) {
        super(context);
        init(context);
        this.adapterPageFlipView = adapterPageFlipView;
    }

    public PageFlipView(Context context, AdapterPageFlipView adapterPageFlipView, AttributeSet attrs) {
        super(context, attrs);
        init(context);
        this.adapterPageFlipView = adapterPageFlipView;
        ...
        if (!mPageFlip.isAnimating()) {
            ...
            try {
                ...
                if (mPageRender != null && mPageRender.onFingerUp(x, y)) {
                    UpdateFromAdapter();
                    requestRender();
                    adapterPageFlipView.checkViewStatus();
//                    setAlpha(0);
                }
            } finally {
                mDrawLock.unlock();
//                setAlpha(0);
    ...
    public boolean is_click_trigger() {
        Log.i(TAG1, adapterPageFlipView.is_click_trigger? "is click": "is move");
        return adapterPageFlipView.is_click_trigger;
    }
    ...
    public void UpdateFromAdapter() {
        if (mPageFlip.getFlipState() == PageFlipState.FORWARD_FLIP) {
            mPageNo++;
            mPageRender.setmPageNo(mPageNo);
            adapterPageFlipView.PostflippedToView(mPageNo);
            ...
            if (mPageFlip.is_back_restore) {
                adapterPageFlipView.onPageFlippedListener.onRestorePage(mPageNo, false);
            }
            mPageNo--;
            mPageRender.setmPageNo(mPageNo);
            Log.i(TAG1, "TMP index -- " + temp_index);
            ...
            adapterPageFlipView.onPageFlippedListener.onRestorePage(mPageNo, true);
        }
    }
