...
public class AdapterPageFlipView extends AdapterView<Adapter> {
    ...
    public AdapterPageFlipView(Context context, AttributeSet attrs) {
        super(context, attrs);
        this.attrs = attrs;
//        IncludeRecyclerView(context);
        IncludePageFlipView(context);
        this.context = context;

    }

    public AdapterPageFlipView(Context context) {
        super(context);
        this.context = context;
        IncludePageFlipView(context);
    }
...
//        for (int i = 1; i <= sideBufferSize; i++) {
        if (bufferedViews.size() >= 1) {
            ...
            //e.g 0     //e.g view 0         //e.g 1    //e.g view 1
            BitmapLoader.get(context).LoadViews_for_Flipping(adapterIndex - 1, previousView, adapterIndex, selectedView,
                    //e.g 2           //e.g view 2
                    adapterIndex + 1, nextView, this);
        }
    }
...
//----------------------------PageFlipView Utils(Our Surface View)----------------------------
    ...
    private PageFlipView IncludePageFlipView(Context context) {
        if (attrs != null) {
...
//                        return false;
                ...
                case MotionEvent.ACTION_MOVE:
                    ...
                    if (can_start_fliping(distance_move)) {
...
//        mPageFlipView.onUserFinger_isDown(getX_position_to_flipBackward(), getY_position_to_flipBackward());
    ...
    private boolean can_start_fliping(float minimum_gap) {
        ViewConfiguration configuration = ViewConfiguration.get(getContext());
        return Math.abs(minimum_gap) > configuration.getScaledTouchSlop();
    }
...
//        context.getResources().getDisplayMetrics().
        return View_Utils.getScreenResolution(getContext()).getHeight() / 3 /*- 300.662f*/;
    }
